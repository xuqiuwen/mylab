...
void arp_data(unsigned char *buffer) {
...
void tcp_header(unsigned char *buffer) {
    ...
    tcpheader_len = (unsigned int)tcp->doff * 4;
    printf("\nTCP Header\n");
...
void TCP_printer(unsigned char *buffer, int buffer_len) {
    ...
    ip_header(buffer);
    tcp_header(buffer);
    unsigned char *data = (buffer + ipheader_len + sizeof(struct ethhdr) + tcpheader_len);
    int data_len = buffer_len - (ipheader_len + sizeof(struct ethhdr) + tcpheader_len);
    payload(data, data_len);
    printf("--------------------------------------------------------------\n\n\n");
}
