...
bool filterByProtocol(unsigned char *buffer, int type) {
    ...
    struct ethhdr *eth = (struct ethhdr *)(buffer);
    int this_type = 0;
    int eth_type = (int)ntohs(eth->h_proto);
    if (eth_type == ETHERTYPE_ARP) {
        this_type = 1;
    } else if (eth_type == ETHERTYPE_IP) {
        struct iphdr *ip = (struct iphdr *)(buffer + sizeof(struct ethhdr));
        int ip_type = ip->protocol;
...
bool filterByPort(unsigned char *buffer, unsigned short port, int type) {
    ...
    if (ip->protocol == IPPROTO_TCP) {
        struct tcphdr *tcp = (struct tcphdr *)(buffer + ip_len + sizeof(struct ethhdr));
        sp = ntohs(tcp->source);
        dp = ntohs(tcp->dest);
    } else if (ip->protocol == IPPROTO_UDP) {
        struct udphdr *udp = (struct udphdr *)(buffer + ip_len + sizeof(struct ethhdr));
        sp = ntohs(udp->source);
        dp = ntohs(udp->dest);
    } else {
        return false;
