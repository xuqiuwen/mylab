...
int main(int argc, char **argv) {
    ...
    int sock_fd = raw_init(device); // 创建混杂模式的原始套接字，捕获的是mac帧
    ...
    while (1) {
        ...
        struct sockaddr sock_addr; // 初始化接收地址内存
        ...
        if (n > 0) {
            if (filterByProtocol(buf, 1) || filterByProtocol(buf, 2) || filterByProtocol(buf, 4)) { // 过滤
                packet_process(buf, n);                                                             // 处理数据包
            }
        } else { // 错误处理
