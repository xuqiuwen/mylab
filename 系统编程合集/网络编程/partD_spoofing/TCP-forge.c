...
unsigned short getchecksum() { // 加上伪首部计算
    ...
    unsigned char buf[sizeof(struct pseudohdr) + sizeof(struct tcphdr)];
    ...
    memcpy(buf + sizeof(struct pseudohdr), th, sizeof(struct tcphdr));
    return checksum(buf, sizeof(struct pseudohdr) + sizeof(struct tcphdr));
}
...
void get_tcp() {
    ...
    th->ack = 0;
    th->urg = 0;
    th->window = htons(20000);
    th->check = htons(getchecksum());
    th->urg_ptr = 0;
