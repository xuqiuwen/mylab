...
unsigned short getchecksum() { // 加上伪首部计算
    struct iphdr *iph = (struct iphdr *)(sendbuff + sizeof(struct ethhdr));
    struct udphdr *uh = (struct udphdr *)(sendbuff + sizeof(struct iphdr) + sizeof(struct ethhdr));
    unsigned char buf[sizeof(struct pseudohdr) + sizeof(struct udphdr)];
    struct pseudohdr *pseudo = (struct pseudohdr *)buf;
    pseudo->source_address = iph->saddr;
    pseudo->dest_address = iph->daddr;
    pseudo->placeholder = 0;
    pseudo->protocol = IPPROTO_UDP;
    pseudo->udp_length = uh->len;
    memcpy(buf + sizeof(struct pseudohdr), uh, sizeof(struct udphdr));
    return checksum(buf, sizeof(struct pseudohdr) + sizeof(struct udphdr));
}
...
void get_udp() {
    ...
    uh->dest = htons(8888);
    uh->len = htons(64 - sizeof(struct ethhdr) - sizeof(struct iphdr));
    uh->check = htons(getchecksum());
