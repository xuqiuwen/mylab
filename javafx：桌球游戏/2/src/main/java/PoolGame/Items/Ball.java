...
public class Ball implements Drawable, Movable {
    ...
    private final double MAX_HIT_FORCE_MAG = RADIUS; // If max vel > radius, vector calculation can be wrong or can miss
                                                     // hitting ball, cause each tick can add > pos of another ball
    ...
    private ImageView cueStickImage; // 台球杆的ImageView
     ...
     * @param pocketAction The action to be carried out when a ball fall into a
     *                     pocket
     * @throws IllegalArgumentException One of the provided value is invalid
     */
    public Ball(String colour, double xPos, double yPos, double xVel, double yVel, double mass, BallType type,
            BallPocketStrategy pocketAction) throws IllegalArgumentException {
    ...
    public void addToGroup(ObservableList<Node> groupChildren) {
        ...
        groupChildren.add(this.cueStickImage);
    }
    ...
    private void initializeCueStickImage() {
        Image image = new Image(getClass().getResourceAsStream("/pole.png")); // 加载图片
        cueStickImage = new ImageView(image);
        cueStickImage.setVisible(false); // 初始设置为不可见
        // 设置其他ImageView属性，如大小和初始位置
    }
        ...
        this.shape.setOnMouseDragged(
                (actionEvent) -> {
                    if (this.hasStopped()) {
                        ...
                        this.mouseDragLine.setEndX(2 * this.shape.getCenterX() - actionEvent.getSceneX());
                        this.mouseDragLine.setEndY(2 * this.shape.getCenterY() - actionEvent.getSceneY());

                        cueStickImage.setVisible(true);
                        double mouseX = actionEvent.getSceneX();
                        double mouseY = actionEvent.getSceneY();

                        // 鼠标位置计算角度
                        double angle = calculateAngle(mouseX, mouseY);
                        cueStickImage.setRotate(angle);

                        // 将台球杆的头部对准鼠标
                        positionCueStickHeadAtMouse(mouseX, mouseY);
                    }
                });
        this.shape.setOnMouseReleased(
                (actionEvent) -> {
                    if (this.hasStopped()) {
                        ...
                        cueStickImage.setVisible(false);
    ...
    public void setReleasedWithSave(Game game) {
        this.shape.setOnMouseReleased(
                (actionEvent) -> {
                    if (this.hasStopped()) {
                        mouseDragLine.setVisible(false);
                        ...
                        // System.err.println(GameState.getInstance().getSize());
                    }
                });
    }

    private void positionCueStickHeadAtMouse(double mouseX, double mouseY) {
        double cueStickLength = cueStickImage.getImage().getHeight(); // 台球杆的实际长度
        double cueStickWidth = cueStickImage.getImage().getWidth(); // 台球杆的实际宽度
        double rotationRadians = Math.toRadians(cueStickImage.getRotate());

        // 计算旋转后台球杆中心的新坐标
        double centerX = mouseX - cueStickWidth / 2;
        double centerY = mouseY - cueStickLength / 2;

        // 计算旋转导致的位移
        double dx = (cueStickLength / 2) * Math.sin(rotationRadians);
        double dy = (cueStickLength / 2) * Math.cos(rotationRadians);

        // 调整台球杆的位置
        double adjustedX = centerX - dx;
        double adjustedY = centerY + dy;

        cueStickImage.setX(adjustedX);
        cueStickImage.setY(adjustedY);
    }

    private double calculateAngle(double mouseX, double mouseY) {
        double dx = mouseX - (this.shape.getCenterX());
        double dy = mouseY - (this.shape.getCenterY());
    ...
    public void handleCollision(Ball ballB) {
        ...
        // The velocity of each ball after a collision can be found by solving the
        // quadratic equation
        // given by equating momentum energy and energy before and after the collision
        // and finding the
        ...
        // only one of the roots is the solution, the other pertains to the current
        // velocities
        if (root - vB < 0.01) {
    ...
    public int getScore() {
        ...
        if (colour.equals(Color.RED)) {
            ...
            return 3;
        } else if (colour.equals(Color.BROWN)) {
            return 3;
        }
        return 0;
    }
